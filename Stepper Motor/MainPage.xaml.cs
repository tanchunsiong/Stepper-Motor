...
namespace Stepper_Motor
{
    public sealed partial class MainPage
    {
        ...
        private void TurnLeft(int degreesToTurn)
        {


        }

        private void TurnRight(int degreesToTurn)
        {

            Task.Run(async () =>
            {

                await _uln2003Driver.TurnAsync(degreesToTurn, TurnDirection.Right);
            });
        }
        ...
        private async void ProcessCurrentVideoFrame(ThreadPoolTimer timer)
        {
            ...
            try
            {
                ...
                else if (alightment >= 500)
                {

                    await _uln2003Driver.TurnAsync(3, TurnDirection.Left);




                }
                else if (alightment <= 300)
                {

                    await _uln2003Driver.TurnAsync(3, TurnDirection.Right);
        ...
        private void SetupVisualization(Windows.Foundation.Size framePizelSize, IList<DetectedFace> foundFaces)
        {
            ...
            if (this.currentState == ScenarioState.Streaming && foundFaces != null && actualWidth != 0 && actualHeight != 0)
            {
                ...
                foreach (DetectedFace face in foundFaces)
                {
                    ...
                    double currentfaceSize = box.Width * box.Height;

                    //if first time
                    if (largestfaceSize == 0)
                    {
                        largestfaceSize = currentfaceSize;
                        alightment = (uint)(face.FaceBox.X / widthScale);

                    }
                    else if (currentfaceSize >= largestfaceSize)
                    {
                        largestfaceSize = currentfaceSize;
                        alightment = (uint)(face.FaceBox.X / widthScale);
                    }
        ...
        /// <param name="e">Event data</param>
        private void CameraStreamingStart()
        {
            if (this.currentState == ScenarioState.Streaming)
            {
